...
# Blueprint and API for authentication
...
# to be done in the main app
jwt = JWTManager()

# Parser for user registration data
...
class RegisterResource(Resource):
    def post(self):
        ...
        try:
            ...
            # Create a cart only if the user is a customer
...
class LoginResource(Resource):
    def post(self):
        ...
        if user and check_password_hash(user.password_digest, data['password']):
            # Convert the role enum to a string representation for JSON serialization
